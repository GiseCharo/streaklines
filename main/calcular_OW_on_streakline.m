...
% Okubo-Weiss
[Q,r2_OW,state_OW,sigma_w2,vort2] = fct_okubo_weiss(grid,v);
Qstreak = interpstreak_any(streakline1,Q,X,Y);
sigma_w2_streak = interpstreak_any(streakline1,sigma_w2,X,Y);

% Tangeante correction
Dtiphi=dtiphistreak(vect_t0,x0,deltat0,t,vel);
...
Dt_norm_Dt0phi = ...
    ( sqrt(sum( dt0phistreak(vect_t0(1:end-1),x0,t+deltat,vel) .^2,2)) - ...
    sqrt(sum( dt0phistreak(vect_t0(1:end-1),x0,t-deltat,vel) .^2,2)) ) ...
    / (2*deltat);
...
Dt_norm_Dtiphi = ...
    ( sqrt(sum( dtiphistreak(vect_t0(1:end-1),x0,deltat0,t+deltat,vel) .^2,2)) - ...
    sqrt(sum( dtiphistreak(vect_t0(1:end-1),x0,deltat0,t-deltat,vel) .^2,2)) ) ...
    / (2*deltat);
